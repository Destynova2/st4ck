...
Sovereign air-gapped Kubernetes platform built on Talos Linux v1.12, deploying a full management cluster with CNI, PKI, identity, monitoring, security, storage, and GitOps stacks across the maintained Scaleway and libvirt/KVM environments, with VMware air-gap kept as a legacy/manual path.
...
State is stored in the bootstrap OpenBao KMS KV v2 via vault-backend (HTTP backend with locking), running in a local podman pod. Secrets are generated by Terraform, stored in encrypted state, seeded into OpenBao Infra, then synchronized by ESO; zero secrets in Git.

Bootstrap uses a single Terraform module (`bootstrap/`) that generates a podman pod manifest with one OpenBao KMS container (single-node Raft), vault-backend, Gitea, Woodpecker, and a tofu-setup sidecar that auto-initializes everything. Authentication to vault-backend uses AppRole (role-id + secret-id from `kms-output/`), not a single token.

## Domain Concepts

- **Platform pod**: Local podman pod running bootstrap OpenBao KMS (single-node Raft) + vault-backend + Gitea + Woodpecker. Single Terraform module in `bootstrap/`. Must run once before any cloud deployment.
