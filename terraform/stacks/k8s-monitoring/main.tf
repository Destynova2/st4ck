...
resource "helm_release" "vm_k8s_stack" {
  ...
  values = [file("${path.module}/../../../configs/vm-k8s-stack/values.yaml")]
...
resource "helm_release" "victoria_logs" {
  ...
  values = [file("${path.module}/../../../configs/victoria-logs/values-single.yaml")]
...
resource "helm_release" "victoria_logs_collector" {
  ...
  values = [file("${path.module}/../../../configs/victoria-logs/values-collector.yaml")]
