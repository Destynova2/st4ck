output "context_id" {
  description = "Context identifier '{namespace}-{env}-{instance}-{region}' — matches the cluster name."
  value       = local.prefix
}

output "region" {
  description = "Region where this cluster lives."
  value       = local.region
}

output "env" {
  description = "Env class."
  value       = local.env
}
...
output "controlplane_ips" {
  ...
  value       = { for name, ip in scaleway_instance_ip.cp : name => ip.address }
}

output "worker_ips" {
  description = "Public IPs of worker nodes."
  value       = { for name, ip in scaleway_instance_ip.wrk : name => ip.address }
}
